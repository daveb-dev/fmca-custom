...
class CovarianceKernel {
 public:
  CovarianceKernel(){};
  CovarianceKernel(const std::string &ktype, FMCA::Scalar l, FMCA::Scalar sigma)
      : ktype_(ktype), l_(l), sigma_(sigma) {
    ...
    ////////////////////////////////////////////////////////////////////////////
        ...
        kernel_ = [this](FMCA::Scalar r) {
        return exp(-2.0 * pow(std::sin(FMCA_PI * r  ), 2.) / l_) * exp(-r * r / l_);
    };
    else if (ktype_ == "PERIODIC")
        kernel_ = [this](FMCA::Scalar r) {
  ...
  FMCA::Matrix eval(const FMCA::Matrix &PR, const FMCA::Matrix &PC) const {
    ...
    for (auto j = 0; j < PC.cols(); ++j)
        for (auto i = 0; i < PR.cols(); ++i)
            retval(i, j) = sigma_* operator()(PR.col(i), PC.col(j));
    return retval;
  }
  
  std::string kernelType() const { return ktype_; }
  
  void setLenght(FMCA::Scalar l) { l_ = l; }
